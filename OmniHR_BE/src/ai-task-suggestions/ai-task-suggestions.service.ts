...
export class AiTaskSuggestionsService {
  ...
  async generate(
    ...
    const [employees, workloads] = await Promise.all([
      this.prisma.employee.findMany({
        where: currentEmployeeWhere({ id: { in: candidateIds }, status: "ACTIVE" }),
        include: {
          department: true,
          position: true,
  ...
  async select(
    ...
    if (suggestion.status !== AiTaskSuggestionStatus.GENERATED) {
      throw new ApiError(
        HttpStatus.BAD_REQUEST,
        "AI suggestion already selected or closed",
  ...
  private present(suggestion: Prisma.AiTaskSuggestionGetPayload<{ include: typeof suggestionInclude }>) {
    return {
  ...
  private proficiencyScore(
    actual?: SkillProficiency | null,
    required?: SkillProficiency | null
  ) {
  ...
  private async availabilityScore(
    employeeId: number,
    startDate?: Date | null,
    dueDate?: Date | null
  ) {
    ...
    const from = startDate ? toDateOnly(startDate) : toDateOnly(new Date());
    const to = dueDate ? toDateOnly(dueDate) : from;
  ...
  private reason(
    fullName: string,
    requiredSkills: Array<{ skill: { code: string }; isRequired: boolean }>,
    ...
    const availabilityText =
      availabilityScore >= 100 ? "không có lịch nghỉ trùng deadline" : "có lịch nghỉ cần cân nhắc";
